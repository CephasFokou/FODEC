...
const LeftSidebar = (props) => {
	...
	const [displayBurgerSideBar, setDisplayBurgerSideBar]=useState(false);
	...
	const updateSideBarStatus = () => {
		setDisplayBurgerSideBar(window.innerWidth <= 1024);
	  };
	
	  // Effect to run when the component is mounted and when the screen is resized so that we will know if we have to use burgerSideBar or not...
	  useEffect(() => {
		updateSideBarStatus();
    ...
    return (
		<>
			...
			{displayBurgerSideBar && isReduced &&
				<svg onClick={()=>setIsReduced(false)} fill='#5DB075' className='w-6 h-6 z-50 absolute left-2 top-5 cursor-pointer  ' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
					<path d="M3 4H21V6H3V4ZM3 11H21V13H3V11ZM3 18H21V20H3V18Z"></path>
				</svg>
			}
			<div className={` ${displayBurgerSideBar ? 'z-50 absolute top-0 left-0 h-screen ' : 'relative'}`}>
				
				{displayBurgerSideBar && !isReduced &&
					<svg onClick={()=>setIsReduced(true)} fill='#5DB075' className='w-6 h-6 z-50 absolute left-2 top-6 cursor-pointer' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
						<path d="M12.0007 10.5865L16.9504 5.63672L18.3646 7.05093L13.4149 12.0007L18.3646 16.9504L16.9504 18.3646L12.0007 13.4149L7.05093 18.3646L5.63672 16.9504L10.5865 12.0007L5.63672 7.05093L7.05093 5.63672L12.0007 10.5865Z"></path>
					</svg>
				}
				<div className="left-sidebar" style={LeftSidebarWidth}>
					{isReduced?
						<div className='logoReducerWrapper'> {/*logo here */}</div>
						:
						<div className="logo flex w-full justify-end sm:justify-center">
							<div className='font-bold text-[22px] sm:text-[20px]  text-center'>
